...
def test_add_consume_bind_code():
    code = add_bind_code("user-1")
    assert len(code) == 6
    assert consume_bind_code(code) == "user-1"
    assert consume_bind_code(code) is None  # 一次性
    assert consume_bind_code("NOTEXST") is None  # 无效


def test_consume_case_insensitive():
    code = add_bind_code("user-ci")
    assert consume_bind_code(code.lower()) == "user-ci"  # 大小写不敏感


def test_add_bind_code_replaces_old():
    c1 = add_bind_code("user-2")
    c2 = add_bind_code("user-2")
    assert c1 != c2
    assert consume_bind_code(c1) is None  # 旧码失效
    assert consume_bind_code(c2) == "user-2"
