...
Two format families:
  ...
  * **Text-like** (plain text, Markdown, source code, JSON, XML, CSV, …) —
    the extension set mirrors the knowledge-base pipeline so the chat
    composer accepts the same text formats. Decoded with a local
    multi-encoding fallback chain (utf-8-sig → utf-8 → gb18030 → latin-1).
...
TEXT_LIKE_EXTENSIONS: frozenset[str] = frozenset({
    ...
    ".csv", ".tsv", ".ini", ".cfg", ".conf", ".env", ".properties",
    ".tex", ".latex", ".bib",
    ".js", ".mjs", ".cjs", ".ts", ".mts", ".cts", ".jsx", ".tsx",
    ".vue", ".svelte",
    ".py", ".rb", ".php", ".pl", ".pm", ".lua", ".r", ".jl", ".dart",
...
def _decode_text(data: bytes) -> str:
    """多编码 fallback 解码：utf-8-sig → utf-8 → gb18030 → latin-1。

    末道用 ``errors="replace"`` 兜底，绝不抛 UnicodeDecodeError（GBK 源码 /
    UTF-8-BOM Markdown 等都能稳定解码）。原实现误调了不存在的
    ``FileTypeRouter.decode_bytes``（该模块既未 import 也无此方法），已改为本地实现。
    """
...
def _extract_text_like(data: bytes, filename: str) -> str:
    ...
    Multi-encoding fallback chain (utf-8-sig → utf-8 → gb18030 → latin-1);
    last resort ``errors="replace"`` so a GBK-encoded Python file or a
    UTF-8-BOM Markdown decodes without raising.
    """
    try:
        return _decode_text(data)
    except Exception as exc:  # pragma: no cover - _decode_text never raises
        raise CorruptDocumentError(
