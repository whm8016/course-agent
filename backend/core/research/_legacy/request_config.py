...
def build_research_runtime_config(
    *,
    ...
    # Use absolute paths relative to this project's data directory
    from pathlib import Path
    from config import BASE_DIR

    system_cfg = dict(runtime_config.get("system", {}) or {})
    system_cfg["output_base_dir"] = str(
        Path(BASE_DIR) / "data" / "research" / "workspace"
    )
    system_cfg["reports_dir"] = str(
        Path(BASE_DIR) / "data" / "research" / "reports"
    )
...
def _build_mode_policy(mode: ResearchMode, depth: ResearchDepth) -> dict[str, Any]:
    from .mode_strategy import get_strategy
