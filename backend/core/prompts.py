...
4. 工具规划：哪些工具可以补足缺失信息（可用工具：rag、web_search）。若信息已足，写"无需工具"。
...
请严格根据 [Thinking]、[Tool Traces] 和 [Observation] 回答，优先引用工具返回的具体事实和证据，不得引入三者中均未出现的信息。
格式要求：
1. 直接给出答案，禁止开场白（"好的"、"当然"、"没问题"等）。
2. 使用 Markdown（标题、加粗、列表）组织结构。
3. 若证据不足，直接说"根据现有资料暂无法确认……"，不要猜测。
4. 不要暴露 thinking / observing / pipeline / tool 等内部字样。"""
