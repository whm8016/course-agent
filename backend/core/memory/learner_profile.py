...
def _summary_prompts(current: str, source: str) -> tuple[str, str]:
    ...
    user = (
        ...
        "## Current Focus\n## Accomplishments\n## Open Questions\n\n"
        "规则：\n"
        "- 整体保持简洁、删除已完成或过时的条目、用短句，不要写逐轮对白。\n"
        "- **若 [Course] 字段与 Current Focus 所描述的课程不同，必须更新 Current Focus 以反映最新学习课程。**\n"
        "- 若本次对话未产生新知识点或新问题，仅更新课程名即可，其余字段可保留。\n\n"
        f"[当前摘要]\n{current or '(empty)'}\n\n"
        f"[新增材料]\n{source}"
    )
    return system, user
...
async def _rewrite_one(which: MemoryFile, current: str, source: str) -> tuple[bool, str]:
    ...
    if not raw:
        logger.warning("memory rewrite empty output which=%s (possible API issue)", which)
        return False, current
    if raw == _NO_CHANGE:
        logger.info("memory rewrite NO_CHANGE which=%s", which)
        return False, current
