"""
Solve BaseAgent — same interface as core/research/base_agent.py for DeepTutor parity.

Loads prompts from core/solve/prompts/{language}/{agent_name}.yaml.
"""
...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        self.logger = logging.getLogger(f"solve.{module_name}.{agent_name}")
    ...
    def _load_prompts(self) -> dict[str, Any] | None:
        prompt_path = (
            Path(__file__).parent / "prompts" / self.language / f"{self.agent_name}.yaml"
        )
    ...
    def refresh_config(self) -> None:
        """Stub for DeepTutor API compatibility."""
    ...
    async def stream_llm(
        ...
        del attachments  # multimodal not wired in solve port v1
